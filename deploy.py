...
def create_version(*,
    ...
    cfg_data['introduction'] = read_lines(__this_dir__ / 'documentation' / 'Introduction.md',
                                          replace_dict={"ONTOLOGY_VERSION": img_version_string.strip("v")})
    ...
    with open(ttl_filename, 'r', encoding='utf-8') as f:
        ...
        for i, l in enumerate(lines):
            if 'owl:versionInfo' in l:
                ...
                _new_line = f"{_splitted_line[0]}owl:versionIRI <https://matthiasprobst.github.io/pivmeta/{version_string.strip('v')}> ;\n"
                lines[i] = _new_line
            ...
            if re.match(r'^\d+(\.\d+)*$', d.name):
                ...
                for i, l in enumerate(lines):
                    if '<dt>Latest version:</dt>' in l:
                        lines[
                            i + 1] = f'            <dd><a href="https://matthiasprobst.github.io/pivmeta">https://matthiasprobst.github.io/pivmeta/{version_string.strip("v")}</a></dd>\n'
...
if __name__ == "__main__":
    prev_version_string = 'v1.1.1'
    version_string = 'v1.1.2'
    img_version_string = 'v1.0.0'
    doi_url = "https://doi.org/10.5281/zenodo.14740584"
    overwrite = True
